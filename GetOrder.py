...
async def confirm_result(callback_query: CallbackQuery, state: FSMContext):
    ...
    async with state.proxy() as data_storage:
        ...
        await callback_query.message.answer(text="Напишие сопроводительное письмо заказчику:",
...
async def send_tz(message: types.Message, state: FSMContext):
    if message.text == "Назад":
        ...
        async with state.proxy() as data_storage:
            index = data_storage["index"]
            Database.add_CoverLetter(data_storage["data"][index][0], CoverLatter, message.from_user.id)
