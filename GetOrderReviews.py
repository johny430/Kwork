...
async def confirm_result(callback_query: CallbackQuery, state: FSMContext):
    ...
    async with state.proxy() as data_storage:
        ...
        if len(reviews) == 0:
            await callback_query.message.answer("На данный заказ еще нет откликов!!!",reply_markup=customer_menu_markup)
            await state.finish()
        else:
...
async def confirm_result(callback_query: CallbackQuery, state: FSMContext):
    ...
    async with state.proxy() as data_storage:
        ...
        url = await Chat.create_group_chat_with_link(f"Заказ номер {customer_id} : {executor_id}")
        await callback_query.message.answer(f'Для начала общения с заказчиком войдите в группу по ссылке:\n{url}', reply_markup=customer_menu_markup)
